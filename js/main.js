document.addEventListener("DOMContentLoaded", () => {
	// 获取所有的pre元素，因为通常代码块会被包裹在pre标签中
	var preElements = document.querySelectorAll("pre");

	// 为每个pre元素设置tab-size样式
	preElements.forEach(function (pre) {
		// 设置CSS属性
		pre.style.setProperty("tab-size", "4");
	});
	...
	const addHighlightTool = () => {
		...
		const expandCode = (e) =>
			e.currentTarget.classList.toggle("expand-done");
	...
	const runJustifiedGallery = (item, data, isButton = false, tabs) => {
		...
		const addButton = (item) => {
			...
			button.innerHTML =
				buttonText + '<i class="fa-solid fa-arrow-down"></i>';
		...
		const completeFn = (e) => {
			...
			btf.loadLightbox(
				item.querySelectorAll("img:not(.medium-zoom-image)")
			);
	...
	const addJustifiedGallery = async (ele, tabs = false) => {
		...
		const init = async () => {
			for (const item of ele) {
				if (btf.isHidden(item) || item.classList.contains("loaded"))
					continue;
	...
	const rightsideScrollPercent = (currentTop) => {
		...
		if (scrollPercent < 95) {
			...
			goUpElement.querySelector(".scroll-percent").textContent =
				scrollPercent;
		} else {
	...
	const scrollFn = () => {
		...
		btf.addEventListenerPjax(window, "scroll", scrollTask, {
			passive: true,
		});
	};
	...
	const scrollFnToDo = () => {
		...
		btf.addEventListenerPjax(window, "scroll", tocScrollFn, {
			passive: true,
		});
	};
	...
	const clickFnOfSubMenu = () => {
		...
		const menusItems = document.querySelector(
	...
	const tabsFn = () => {
		...
		const handleNavClick = (e) => {
			...
			const tabContents = [
				...e.currentTarget.nextElementSibling.children,
			];
			...
			addJustifiedGallery(
				tabContents[indexOfButton].querySelectorAll(
					".gallery-container"
				),
				true
			);
	...
	const addPostOutdateNotice = () => {
		...
		const { limitDay, messagePrev, messageNext, postUpdate } = JSON.parse(
	...
	const justifiedIndexPostUI = () => {
		...
		if (
			!(
				recentPostsElement &&
				recentPostsElement.classList.contains("masonry")
			)
		)
			return;
